...
class TMCM1110(TMCLModule):
    """
    The TMCM-1110 is a single axis stepper motor controller/driver module for sensorless load dependent current control.
    ...
    class _MotorTypeA(MotorControlModule):
        ...
        class AP:
            ...
            # BoostCurrent                = 200 -> not active in TMCL-IDE 3.6.0.0 (but listed in manual rev. v1.08)
            FreewheelingDelay           = 204
            LoadValue                   = 206
            # ExtendedErrorFlags          = 207 -> not active in TMCL-IDE 3.6.0.0 (but listed in manual rev. v1.08)
            DrvStatusFlags              = 208
            EncoderPosition             = 209
            EncoderPrescaler            = 210
            MaxEncoderDeviation         = 212
            PowerDownDelay              = 214
            # StepDirMode(?)              = 254 -> not active in TMCL-IDE 3.6.0.0 (but listed in manual rev. v1.08)
    ...
    class GP0:
        SerialBaudRate         = 65
        SerialAddress          = 66
        # ASCIImode              = 67 -> not listed in manual rev. v1.08 but active in TMCL-IDE 3.6.0.0
        SerialHeartbeat        = 68
        CANBitRate             = 69
        CANsendID              = 70
        CANreceiveID           = 71
        ...
        # StepPulseLength(?)     = 86 -> listed in manual rev. v1.08 and TMCL-IDE 3.6.0.0
        serialSecondaryAddress = 87
        # reverseShaftDirection  = 90 -> not active in TMCL-IDE 3.6.0.0 (but listed in manual rev. v1.08)
        ApplicationStatus      = 128
        downloadMode           = 129
        ProgramCounter         = 130
        # LastError              = 131 -> not listed in manual rev. v1.08 but active in TMCL-IDE 3.6.0.0
        TickTimer              = 132
        randomNumber           = 133
        # SuppressReply          = 255 -> not active in TMCL-IDE 3.6.0.0 (but listed in manual rev. v1.08)
